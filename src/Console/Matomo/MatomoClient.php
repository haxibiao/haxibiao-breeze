...
class MatomoClient extends Command
{
     ...
     * @var string
     */
    protected $signature = 'matomo:client {--server=localhost} {--siteid=}　{--test} {--port=}';
     ...
     * @var string
     */
    protected $description = '测试 matomo:proxy 用';
    ...
    public function handle()
    {
        ...
        go(function () use ($data, $port) {
            ...
            //下面的请求在 php-fpm 下可以，必须同步
            $server = $this->option('server');
            // $server = "127.0.0.1";
            $this->info("test tcp to $server");
            try {
                $client = new \swoole_client(SWOOLE_SOCK_TCP); //tcp 默认timeout 0.1s
                $client->connect($server, $port) or die("swoole connect failed\n");
                $this->info('connect ...');
                $client->set([
                    'open_length_check'     => true,
                    'package_length_type'   => 'n',
