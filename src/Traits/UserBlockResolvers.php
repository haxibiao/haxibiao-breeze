...
trait UserBlockResolvers
{
    ...
    public function addUserBlock($rootValue, array $args, GraphQLContext $context, ResolveInfo $resolveInfo)
    {
        if ($user = checkUser()) {
            ...
            //跳过已经屏蔽过的用户
            $existUser = UserBlock::where("user_id", $user->id)->where("user_block_id", $args['id'])->get();
            if ($existUser->count()) {
    ...
    public function addArticleBlock($rootValue, array $args, GraphQLContext $context, ResolveInfo $resolveInfo)
    {
        if ($user = checkUser()) {
            ...
            //跳过已经屏蔽过的用户
            $existUser = UserBlock::where("user_id", $user->id)->where("article_block_id", $args['id'])->get();
            if ($existUser->count()) {
    ...
    public function reportArticle($rootValue, array $args, $context, $resolveInfo)
    {
        ...
        try {
            ...
            if ($ReportCount >= 2) {
                ...
                $user         = $post->user;
                //智慧点或贡献点不够   扣则置为0
                $gold = Gold::makeOutcome($user, $user->gold - Gold::REWARD_GOLD < 0 ?
                    $user->gold : Gold::REWARD_GOLD, '动态被举报下架扣除');
                $gold->save();
                ...
                //更新profile表上的字段
                UserProfile::where('id', $user->profile->id)->update(['count_contributes' => $count]);
