                ...
                <div class="content">
                    ...
                    <div class="article-list tab-content">
                        <ul role="tabpanel" class="fade in active  user-list follow-user-list tab-pane" id="following">
                            ...
                            @endforeach
                            @if (Auth::check())
                                <follow-user-list api="/api/user/{{ $user->id }}/follow?followings=1" start-page="2"
                                    not-empty="{{ count($data['follows']) > 0 }}" current-user-id="{{ user_id() }}">
                                </follow-user-list>
                            @endif
                        </ul>
                        <ul role="tabpanel" class="fade user-list follow-user-list tab-pane" id="followers">
                            ...
                            @endforeach
                            @if (Auth::check())
                                <follow-user-list api="/api/user/{{ $user->id }}/follow?followers=1" start-page="2"
                                    not-empty="{{ count($data['followers']) > 0 }}" current-user-id="{{ user_id() }}">
                                </follow-user-list>
                            @endif
                        </ul>
                    </div>
