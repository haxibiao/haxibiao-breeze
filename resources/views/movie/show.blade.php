...
@push('head-styles')
    <link rel="stylesheet" href="{{ asset('/vendor/breeze/css/movie/play.css') }}">
@endpush
...
@section('content')
    <div class="app-player">
        <div class="container-xl">
            @if (checkUser())
                <movie-player :movie="{{ $movie }}" count="{{ count($movie->data) }}"
                        ...
                        @foreach ($more as $movie)
                            <div class="rc-item col-lg-12 col-sm-6 clearfix">
                                ...
                                <div class="info-wrapper">
                                    <a href="/home" target='_blank' class="video-title webkit-ellipsis"
                                        title="{{ $movie->name }}">
                                        {{ $movie->name }}
                                    </a>
                                    <div class="video-count">
                                        {{ rand(1, 99) }}.{{ rand(0, 9) }}万播放&nbsp;&nbsp;·&nbsp;&nbsp;1万评论</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
...
@push('foot-scripts')
    <script src="{{ asset('/vendor/breeze/js/movie/play.js') }}" defer></script>
@endpush
